    ...
    def inquiryBalance(self):
     ...
     #   self.inquiryTimer.stop()
        ...
        try:
            ...
            self.kiwoom.setInputValue("비밀번호", self.user_param['password'])
            self.kiwoom.commRqData("예수금상세현황요청", "opw00001", 0, "0101")

            # 계좌평가잔고내역요청 - opw00018 은 한번에 20개의 종목정보를 반환
            self.kiwoom.setInputValue("계좌번호", self.kiwoom.account)
            self.kiwoom.setInputValue("비밀번호", self.user_param['password'])
            self.kiwoom.commRqData("계좌평가잔고내역요청", "opw00018", 0, "0101")
